...
class Camera:
    ...
    def releaseCamera(self):
        ...
        try:		
            ...
            subprocess.run(killGphoto, check=True, stdout=subprocess.PIPE, universal_newlines=True)
            print(' - Gphoto2 killed')
            errorTrace = 'kill kernel'

            killKernel = ['pkill', '-f', 'gvfsd-gph']
            subprocess.run(killKernel, check=True, stdout=subprocess.PIPE, universal_newlines=True)
            print(' - Kernel killed')
            errorTrace = 'target'

            setTarget  = ['gphoto2', '--set-config', 'capturetarget=1']
            subprocess.run(setTarget, check=True, stdout=subprocess.PIPE, universal_newlines=True)
            print(' - Targed setted')
    ...
    def trigger_capture_cmd(self):
        ...
        try:
            ...
            subprocess.run(fastCapture, check=True, stdout=subprocess.PIPE, universal_newlines=True)
        except subprocess.CalledProcessError:
            print("Error")
